...
void transferExcesses(const MapF& terrain, MapF& flowingWater,
                      const Map<Index>& sinks,
                      std::map<Index, Basin>& basins,
...
auto lowestNeighborByTotalHeight(const MapF& terrain, const MapF& water, Index index)
{
...
std::pair<MapF, MapF> addWater(const MapF& terrain, const MapF& precipitation,
                               float depthThreshold)
{
    ...
    MapF flowingWater = precipitation;
    ...
    MapF stationaryWater(terrain.size(), 0);
    for (auto& [sink, basin] : basins) {
...
auto lowestMooreNeighbor(const MapF& totalHeight, const Index& c)
{
...
auto strahlerNumbers(const MapF& totalHeight, std::vector<Index> sortedByTotalHeight)
{
...
void addRivers(MapF& terrain, MapF& water)
{
    ...
    for (std::size_t x = 0; x < strahlerNumber.width(); ++x)
        for (std::size_t y = 0; y < strahlerNumber.height(); ++y) {
            if (water[x][y] > 0)
                continue;
            float riverDepth = float(strahlerNumber[x][y] - min) / (max - min);
            if (riverDepth > 0.36f) {
                riverDepth = std::pow(riverDepth - 0.08f, 0.3f) * 0.06f;
